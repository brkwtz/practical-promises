...
function problemA () {
  ...
  // promise version
  promisifiedReadFile('./poem-one/stanza-01.txt').then(function successHandler1 (stanza) {
    // use the value inside this callback
    blue('async1 is done!', stanza);
...
function problemB () {
  ...
  // promise version
  promisifiedReadFile('./poem-one/stanza-02.txt').then(function successHandler1 (stanza) {
      // use the value inside this callback
      blue('stanza 2 is done!', stanza);
    }).catch(function errorHandler (err) {
      magenta('ERR', err);
    });

  promisifiedReadFile('./poem-one/stanza-03.txt').then(function successHandler1 (stanza) {
      // use the value inside this callback
      blue('stanza 3 is done!', stanza);
    }).catch(function errorHandler (err) {
      magenta('ERR', err);
    });
...
function problemC () {
  ...
  promisifiedReadFile('./poem-one/stanza-02.txt')
    ...
    .then(function successHandler2 (stanza) {
      blue(stanza);
    })
    .catch(function errorHandler (err) {
      magenta('ERR', err);
...
function problemD () {
  ...
  // promise version
  promisifiedReadFile('poem-one/stanza-02344.txt')
  .then((stanza) => {
    blue(stanza);
  })
  .catch((err) => {
